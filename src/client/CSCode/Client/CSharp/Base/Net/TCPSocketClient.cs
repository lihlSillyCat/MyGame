    ...
    public class TCPSocketClient:  ISocketClient
    {
        ...
        private void ThreadSend()
        {
            ...
            while (!m_Abort && IsConnected())
            {
                ...
                if (null != node)
                {
                    short nSize = (short)node.item.Pack(sendBuffer, MESSAGE_HEAD_LEN);
                    UnityEngine.Debug.LogError("nSize: " + nSize);
                    short nSizeNet = IPAddress.HostToNetworkOrder(nSize);

                    byte[] packetSizeBytes = BitConverter.GetBytes(nSizeNet);
